...
- **`src/modules/auth/`** (Etapas 2 y 5, ver `docs/DECISIONS.md` y `docs/API.md`) — `POST /api/auth/session` (primer acceso: vincula `firebaseUid` ↔ `User` preprovisionado por email, ver `sessionService.ts`), `GET /api/auth/me` (perfil mínimo del usuario ya vinculado) y `GET /api/auth/admin-check` (solo técnica/temporal, valida `requireRole("ADMIN")` de punta a punta). Montados sin flag en `/api/auth` (`app.ts`). **Ninguna ruta de negocio existente está protegida todavía** — `/check-in`, registro General, VIP y Mercado Pago siguen exactamente igual que antes, y ninguna ruta de `auth/` interactúa con Mercado Pago en ningún punto. No hay registro público de `ADMIN`/`VALIDATOR`: solo puede vincularse un `User` preprovisionado de antemano (`backend/scripts/createStaffUser.ts`).
