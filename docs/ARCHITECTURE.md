...
- **`src/middlewares/requireAuth.ts` / `requireRole.ts`** — infraestructura de autenticación y autorización para `ADMIN`/`VALIDATOR` (Etapa 1, ver `docs/DECISIONS.md`). `requireAuth` valida el Firebase ID Token del header `Authorization: Bearer <token>` (vía `integrations/firebase/firebaseAdmin.ts`), exige `email_verified === true`, resuelve el `User` interno por `firebaseUid` y adjunta `req.authUser` (`firebaseUid`, `email`, `emailVerified`, `userId`, `role`, `status` — tipado en `authTypes.ts`, `Request` aumentado en `src/types/express.d.ts`); `role`/`status` siempre se leen de esa fila de Postgres, nunca del token. `requireRole(...roles)` solo lee `req.authUser.role` ya resuelto.
- **`src/modules/auth/`** (Etapa 2, ver `docs/DECISIONS.md` y `docs/API.md`) — primer consumidor real de `requireAuth`/`requireRole`: `GET /api/auth/me` (perfil mínimo del usuario autenticado) y `GET /api/auth/admin-check` (solo técnica/temporal, valida `requireRole("ADMIN")` de punta a punta). Montado sin flag en `/api/auth` (`app.ts`). **Ninguna ruta de negocio existente está protegida todavía** — `/check-in`, registro General, VIP y Mercado Pago siguen exactamente igual que antes, y ninguna de las dos rutas de `auth/` interactúa con Mercado Pago en ningún punto. Vinculación `firebaseUid` ↔ `User` por email (`POST /api/auth/session`) sigue pendiente para una próxima etapa.
...
- **Firebase** (`integrations/firebase/firebaseAdmin.ts`) — Admin SDK, solo para autenticar administradores y validadores (nunca a los asistentes que compran/registran entradas, que siguen sin cuenta). Implementado: inicialización perezosa (un solo import no tiene efecto secundario), normalización de `FIREBASE_PRIVATE_KEY` (saltos de línea literales `\n` → reales), y verificación de ID Tokens con `checkRevoked: true`. Todavía no implementado: los endpoints que lo consumen (`/api/auth/session`, `/api/auth/me`) ni ninguna ruta protegida — ver `docs/DECISIONS.md` y `SESSION_HANDOFF.md`.
