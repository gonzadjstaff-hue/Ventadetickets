...
## Diagnóstico de producción: el webhook de Mercado Pago descartaba su propio resultado sin loguearlo

**Síntoma reportado:** en producción, un pago sandbox se aprobaba en Mercado Pago (el frontend recibía `status=approved` al volver), pero la `Order` quedaba `PENDING` — el webhook real "aparentemente" no la actualizaba, sin ninguna forma de saber por qué.

**Causa raíz confirmada por lectura de código (no supuesta):** `backend/src/modules/payments/mercadoPagoController.ts`, el handler de `POST /api/webhooks/mercadopago`, llamaba a `processMercadoPagoWebhook(...)` y **descartaba por completo su valor de retorno** (`WebhookOutcome`: `applied` / `already_processed` / `ignored` con motivo `order_not_found`/`amount_mismatch`/`currency_mismatch`/`live_mode_mismatch`), respondiendo siempre `200 { received: true }` sin importar cuál de esos resultados se hubiera dado — y **sin loguear nada** en todo el flujo (`mercadoPagoWebhookService.ts` no tenía un solo `console.*`). Esto significa que el sistema no tenía forma de distinguir "el webhook nunca llegó", "llegó pero se ignoró por un mismatch real" o "se procesó bien" — los tres casos eran indistinguibles desde afuera. Además, `verifySignature` (`backend/src/integrations/payments/mercadoPago/mercadoPagoClient.ts`) atrapaba cualquier excepción del validador del SDK (incluida la típica `InvalidWebhookSignatureError`, que trae un campo `.reason` tipado — `MissingSignatureHeader`/`MalformedSignatureHeader`/`MissingHash`/`SignatureMismatch`/etc., pensado explícitamente por el autor del SDK para loguearse) y la descartaba sin dejar rastro, devolviendo solo `false`.

El código de aprobar/ignorar (importe, moneda, `live_mode`, `external_reference`) ya era correcto — el defecto real era de **observabilidad**, no de lógica de negocio: el sistema podía estar funcionando bien o mal y no había manera de saberlo desde los logs de Render.

**Corrección aplicada — solo logging, sin cambiar ningún código de respuesta HTTP existente** (los códigos ya eran semánticamente correctos: 401 firma inválida, 200 aplicado/ignorado/ya-procesado — un "ignorado" es una condición permanente que reintentar no resuelve, ver más arriba —, 5xx error interno para que Mercado Pago reintente):

- `mercadoPagoClient.ts` / `verifySignature`: loguea el motivo exacto (`error.reason` del SDK) en cada rechazo de firma, y distingue "sin `MERCADOPAGO_WEBHOOK_SECRET` configurado" de "firma genuinamente inválida" — nunca loguea el header `x-signature` crudo ni el secret.
- `mercadoPagoWebhookService.ts`: logs en cada punto de decisión (notificación recibida, pago consultado con su `status`/`liveMode`, motivo exacto de cada `ignored` con los valores comparados, transición final aplicada) — nunca el email del comprador ni nombres de asistentes (mismo criterio que el resto del archivo, ver más arriba).
- `mercadoPagoController.ts`: loguea el resultado de la validación de firma y, en el `catch`, el error que antes se traducía a una respuesta HTTP en silencio total.

**Tests nuevos** (`backend/tests/mercadoPagoWebhookRealSignature.test.ts`): a diferencia de `mercadoPagoWebhook.test.ts` (que mockea todo `mercadoPagoProvider`, incluida `verifyWebhookSignature`), este archivo deja el validador de firma **real** (el mismo código del SDK que corre en producción) y solo reemplaza las llamadas de red (`getPayment`/`createPreference`/`getPreference`), con headers/body representativos de un webhook real — cerrando exactamente el hueco de cobertura que permitía que una firma real rechazada pasara desapercibida mientras la suite (contra el provider mockeado) seguía en verde.
